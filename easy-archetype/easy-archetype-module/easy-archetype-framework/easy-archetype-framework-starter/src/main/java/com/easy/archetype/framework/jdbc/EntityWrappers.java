...
public class EntityWrappers<T> {
	...
	public EntityWrappers(T entity, JdbcGlobalConfig jdbcGlobalConfig,
	...
	public SqlResult getSelectSql() {
		...
		if (CollectionUtil.isEmpty(entityWrapper.getSelectSql())) {
			...
			sql.append(entityInfo.getFieldInfoList().stream().map(EntityFieldInfo::getJdbcField).collect(Collectors.joining(",")));
		} else {
	...
	public void setWhereSql(StringBuilder sql, FieldStrategy globalFieldStrategy, boolean ignoreId) {
		...
		if (null != jdbcGlobalConfig && !globalFieldStrategy.equals(FieldStrategy.DEFAULT)) {
			Map<String, Object> fields = jdbcEntityParser.getFieldByStrategy(entityInfo.getFieldInfoList(), globalFieldStrategy, ignoreId);
	...
	public SqlResult getInsertSql() {
		...
		// 根据策略获取所有需要插入的字段
		Map<String, Object> fields = jdbcEntityParser.
				getFieldByStrategy(entityInfo.getFieldInfoList(),
		...
		StringBuilder sql = new StringBuilder();
		sql.append(" INSERT  INTO ").append(entityInfo.getTableName())
				.append("(").append(fields.keySet().stream().collect(Collectors.joining(",")))
				.append(")").append(" VALUES (")
				.append(fields.values().stream().map(a -> "?").collect(Collectors.joining(",")))
				.append(")");
	...
	public SqlResult getUpdateSql() {
		...
		Map<String, Object> fields = jdbcEntityParser.getFieldByStrategy(entityInfo.getFieldInfoList()
				...
				this.jdbcGlobalConfig.getUpdateStrategy(), true);
	...
	public static class SqlResult<T> {
		...
		public static <T> SqlResult<T> builder(String sql, List<Object> args, Class<?> classType, EntityInfo<T> entityInfo) {
