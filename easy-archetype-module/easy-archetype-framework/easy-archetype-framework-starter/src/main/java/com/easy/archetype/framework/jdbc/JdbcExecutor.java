...
public class JdbcExecutor extends JdbcTemplate {
	...
	public <T> List<T> selectBatchIds(Collection<? extends Serializable> idList, Class<T> requiredType) {
		...
		entityWrapper.in(false, getEntityWrappers(requiredType).getEntityInfo().getIdFieldInfo().getFieldName(), idList);
		EntityWrappers.SqlResult sqlResult = getEntityWrappers(entityWrapper).getSelectSql();
	...
	public <T> Integer selectCount(EntityWrapper<T> wrapper) {
		...
		return this.queryForObject(countSql.getSql(), Integer.class,
				countSql.args());
	}
	...
	public <T> int insert(T entity) {
		...
		int update = this.update(con -> {
			// 设置返回的主键字段名
			PreparedStatement ps = con.prepareStatement(insertSql.getSql(), Statement.RETURN_GENERATED_KEYS);
		...
		// 主键id反射赋值
		if (CollectionUtil.isNotEmpty(keyHolder.getKeyList()) && null != insertSql.getEntityInfo().getIdFieldInfo()) {
	...
	public <T> int deleteById(Serializable id, Class<T> requiredType) {
		...
		sql.append(" WHERE " + SqlKeyword.EQ.getSqlSegment(entityWrappers.getEntityInfo().getIdFieldInfo().getJdbcField()));
	...
	public <T> int delete(EntityWrapper<T> wrapper) {
		EntityWrappers<T> entityWrappers =
				getEntityWrappers(wrapper);
	...
	public <T> int deleteBatchIds(Collection<? extends Serializable> idList, Class<T> requiredType) {
		...
		entityWrapper.in(CollectionUtil.isNotEmpty(idList), entityWrappers.getEntityInfo().getIdFieldInfo().getJdbcField(),
				idList);
