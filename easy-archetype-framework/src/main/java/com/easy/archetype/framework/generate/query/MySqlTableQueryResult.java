...
public class MySqlTableQueryResult implements ITableQueryResult {
    ...
    public List<TableFieldEntity> getTableFieldEntitys(String tableName, DataSource dataSource) {
        ...
        try {
            List<TableFieldEntity> query = Db.use(dataSource).query(TABLE_FIELD_SQL, new RsHandler<List<TableFieldEntity>>() {
                ...
                public List<TableFieldEntity> handle(ResultSet rs) throws SQLException {
                    ...
                    while (rs.next()) {
                        ...
                        // 字段类型·
                        tableFieldEntity.setJdbcFieldName(rs.getString("COLUMN_NAME") == null ? "" : rs.getString("COLUMN_NAME"));
                        // 字段类型
                        ...
                        tableFieldEntity.setNullable("no".equalsIgnoreCase(rs.getObject("IS_NULLABLE").toString()) ? Boolean.FALSE : Boolean.TRUE);
                        // 默认值
                        tableFieldEntity.setColumnDefault(rs.getString("COLUMN_DEFAULT") == null ? "" : rs.getString("COLUMN_DEFAULT"));
                        // 编码
                        tableFieldEntity.setJdbcColumnCharacter(rs.getString("CHARACTER_SET_NAME") == null ? "" : rs.getString("CHARACTER_SET_NAME"));
                        //  字段长度
                        tableFieldEntity.setJdbcLength(rs.getString("COLUMN_TYPE")
                                .toUpperCase()
                                .replace(rs.getString("DATA_TYPE").toUpperCase(), ""));
                        //是否为主键
                        tableFieldEntity.setPrimaryKey("PRI".equals(rs.getString("COLUMN_KEY")) ? Boolean.TRUE : Boolean.FALSE);
                        //  是否自增
                        tableFieldEntity.setIncrement("auto_increment".equalsIgnoreCase(rs.getString("EXTRA")) ? Boolean.TRUE : Boolean.FALSE);
                        // 注释
                    ...
                    while (resultSet.next()) {
                        ...
                        if (map.containsKey(keyName)) {
                            ...
                            tableIndexEntity = TableIndexEntity
                                    ...
                                    //索引类型
                                    .indexType(resultSet.getString("Index_type") == null ? "" : resultSet.getString("Index_type"))
                                    //索引方法
                                    .indexMethod(resultSet.getInt("Non_unique") == 0 ?
                                            "UNIQUE" : "NORMAL")
                                    // 索引注释
