...
public class GenerateBootstrap {
     ...
     * @since 2021/2/2
     */
    public GenerateBootstrap columnTypeConverRegistry(FactoryRegistryHandler<Map<Class<? extends Driver>, IColumnTypeConver>> factoryRegistryHandler) {
     ...
     * @since 2021/2/2
     */
    public GenerateBootstrap tableQueryResultRegistry(FactoryRegistryHandler<Map<Class<? extends Driver>, ITableQueryResult>> factoryRegistryHandler) {
     ...
     * @since 2021/2/2
     */
    public GenerateBootstrap templateGroupRegister(FactoryRegistryHandler<Map<Class<? extends ITemplateGroup>, ITemplateGroup>> factoryRegistryHandler) {
    ...
    public void generate(Class<? extends ITemplateGroup>... templateGroupClass) {
        ...
        for (Class<? extends ITemplateGroup> groupClass : templateGroupClass) {
            ...
            // 6. 查询出来所有需要处理的表
            List<TableInfoEntity> tableInfoEntities = templateGroup.getGenerateTableInfo(tableQueryResult, globalConfig, converFactoryConver);
            System.out.println(groupClass.getName() + "模板组: 待生成的表的数量为:" + tableInfoEntities.size());
            List<OutputFile> outputFiles = templateGroup.generate(tableInfoEntities, this.templateEngine, this.globalConfig);
            // 交给文件处理器处理
