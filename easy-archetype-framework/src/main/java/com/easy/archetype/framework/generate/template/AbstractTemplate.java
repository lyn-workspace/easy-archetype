...
public abstract class AbstractTemplate implements ITemplate {
    ...
    private TableFieldEntity getIdField(TableInfoEntity tableInfoEntity) {
        ...
        TableFieldEntity tableFieldEntity = tableFieldEntities.stream().filter(a -> a.getPrimaryKey().equals(Boolean.TRUE))
                .findFirst()
                //当找不见主键的时候,默认寻找第一个不为时间类型的字段作为主键
                .orElse(tableFieldEntities
                        .stream()
                        .filter(a -> !a.getJdbcType().contains("time") || !a.getJdbcType().contains("date")).findFirst().orElseThrow(() -> {
    ...
    public String classPkg() {
        return (this.globalConfig.getParentPkg() + "." +
                module() + "." + pkg())
                .replaceAll("\\.+", ".");
    }
